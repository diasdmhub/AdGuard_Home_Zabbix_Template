...
- AdGuard Home
  ...
  > _**The template uses the `system.run[*]` key for active monitoring with the Zabbix Agent. The `AllowKey=system.run[*]` parameter can be enabled to allow the agent to collect data locally.**_
